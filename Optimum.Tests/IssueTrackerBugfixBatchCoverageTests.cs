...
public class IssueTrackerBugfixBatchCoverageTests
{
    ...
    public void FallingBlockRendererDoesNotMutateTheSharedDefaultBlockMesh()
    {
        ...
        // The upload goes directly from the local mesh variable, not through a reusable field
        Assert.Contains("UploadMultiTextureMesh(mesh)", source);
        Assert.DoesNotContain("this.mesh = capi.TesselatorManager.GetDefaultBlockMesh", source);
    }
