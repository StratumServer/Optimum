...
public class LatencyRendererCoverageTests
{
    ...
    {
        string device = Read(DevicePath) + Read(DeviceLatencyPath);
        ...
        string beginFrame = Body(Read(DevicePath), "    public void BeginFrame()");
    ...
    {
        string device = Read(DevicePath) + Read(DeviceLatencyPath);
        string present = Body(Read(DevicePath), "    public void Present()");
    ...
    public void PresentIdsAreEnabledWithoutAPacingBackendAndOnlyNoneIsConstructed()
    {
        ...
        Assert.Contains("_swapchain!.PresentIdEnabled = _context.Capabilities.PresentIdEnabled;", Read(DevicePath));
    }
