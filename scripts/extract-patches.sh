...
if [[ -d "$patches_dir" ]]; then
  ...
  if [[ -d "$patches_dir/runtime" ]]; then
    mkdir -p "$preserved_dir/patches/runtime"
    cp -a "$patches_dir/runtime/." "$preserved_dir/patches/runtime/"
  fi
fi
...
for base_project in "$baseline_dir"/*/; do
  ...
  while IFS= read -r -d '' file; do
    ...
    # Bootstrap creates VintagestoryLib.csproj from the real binary
    # contract and must never become a Cecil/source patch.
    if [[ "$project" == "VintagestoryLib" && "$rel" == "VintagestoryLib.csproj" ]]; then
      continue
    fi
...
while IFS= read -r -d '' patch; do
  ...
  -not -path "$patches_dir/runtime/*" -print0 2>/dev/null)

bash "$script_dir/validate-patch-syntax.sh" "$patches_dir"
