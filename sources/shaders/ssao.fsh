...
#if TAAMOTION == 1
// Optimum (GTAO roadmap step 2): the temporal pipeline's own frame counter -
// OptimumTemporal.Frame.FrameIndex, wrapped so it stays exact in a float. One
// clock for the whole pipeline: the camera jitter, the TAA resolve and the
// upscaler all step on this index, and the dither below steps with them.
// Declared only while a temporal consumer owns the frame, so the SSAO pass can
// leave it unset and this file still preprocesses back to vanilla.
